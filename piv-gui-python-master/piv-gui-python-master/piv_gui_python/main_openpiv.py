...
class Main(QtGui.QWidget):
    def __init__(self):
        ...
        self.frame_size_x = imageio.imread(self.raw_frames[0]).shape[1]
        self.frame_size_y = imageio.imread(self.raw_frames[0]).shape[0]
        ...
        self.ui.iw_pos_x.setMaximum(
            (self.frame_size_x - self.iw_big_size_value)/self.iw_big_size_value)
        self.ui.iw_pos_y.setMaximum(
            (self.frame_size_y - self.iw_big_size_value)/self.iw_big_size_value)
    ...
    def set_slider(self, delta):
        ...
        self.ui.main_slider_value.setValue(
            self.ui.main_slider_value.value() + delta * self.ui.main_slider_value.singleStep())
    ...
    def update_iw_slider_max(self):
        ...
        self.ui.iw_pos_x.setMaximum((scipy.misc.imread(
            self.raw_frames[0]).shape[1] - self.iw_big_size_value)/(self.iw_big_size_value - self.overlap_size_value))
        self.ui.iw_pos_y.setMaximum((scipy.misc.imread(
            self.raw_frames[0]).shape[0] - self.iw_big_size_value)/(self.iw_big_size_value - self.overlap_size_value))
    ...
    def draw_iw_bounds(self, main_pixmap):
        ...
        draw.drawRect(self.iw_pos_x_value + (self.iw_big_size_value - self.iw_small_size_value)/2, self.iw_pos_y_value +
                      (self.iw_big_size_value - self.iw_small_size_value)/2, self.iw_small_size_value, self.iw_small_size_value)
        draw.setPen(QtCore.Qt.red)
        # Draw the big IW
        draw.drawRect(self.iw_pos_x_value, self.iw_pos_y_value,
                      self.iw_big_size_value, self.iw_big_size_value)
        draw.setPen(QtCore.Qt.green)
        # Draw a point in the centre of the two IWs.
        draw.drawPoint(self.iw_big_size_value/2 + self.iw_pos_x_value,
                       self.iw_big_size_value/2 + self.iw_pos_y_value)
    ...
    def display_small_iw(self):
        ...
        frame_a = cv2.imread(self.raw_frames[self.main_slider_value])
        self.small_windows = self.moving_sub_window_array(
            frame_a, self.iw_big_size_value, self.overlap_size_value, self.subwindow_inset)

        # field shape provides the information how many big IWs fit in the frame of interest.
        self.field_shape = openpiv.process.get_field_shape(image_size=scipy.misc.imread(
            self.raw_frames[self.main_slider_value]).shape, window_size=self.iw_big_size_value, overlap=self.overlap_size_value)

        # TODO: try and make this more readable. Automate scaling?
        self.pixmap_small_iw = QtGui.QPixmap.fromImage(ImageQt.ImageQt(scipy.misc.toimage(self.small_windows[self.iw_pos_x_value/(self.iw_big_size_value - self.overlap_size_value) + (
            self.iw_pos_y_value/(self.iw_big_size_value - self.overlap_size_value))*self.field_shape[1]], cmax=255, cmin=0))).scaled(64, 64, QtCore.Qt.KeepAspectRatio)
    ...
    def display_big_iw(self):
        ...
        self.big_windows = self.moving_window_array(
            frame_b, self.iw_big_size_value, self.overlap_size_value)

        # TODO: try and make this more readable. Automate scaling?
        self.pixmap_big_iw = QtGui.QPixmap.fromImage(ImageQt.ImageQt(scipy.misc.toimage(self.big_windows[self.iw_pos_x_value/(self.iw_big_size_value - self.overlap_size_value) + (
            self.iw_pos_y_value/(self.iw_big_size_value - self.overlap_size_value))*self.field_shape[1]], cmax=255, cmin=0))).scaled(128, 128, QtCore.Qt.KeepAspectRatio)
    ...
    def replot_iws(self):
        ...
        if self.corr_mat_plt and self.corr_mat_plt.isVisible():
            calc_cor_matrix = openpiv.process.correlate_windows(window_a=self.small_windows[self.iw_pos_x_value/(self.iw_big_size_value - self.overlap_size_value) + (self.iw_pos_y_value/(self.iw_big_size_value - self.overlap_size_value))*self.field_shape[1]].astype(
                np.float64), window_b=self.big_windows[self.iw_pos_x_value/(self.iw_big_size_value - self.overlap_size_value) + (self.iw_pos_y_value/(self.iw_big_size_value - self.overlap_size_value))*self.field_shape[1]].astype(np.float64), corr_method='fft', nfftx=None, nffty=None)
    ...
    def plot_corr_mat_btn_clicked(self):
        ...
        self.calc_cor_matrix = openpiv.process.correlate_windows(window_a=self.small_windows[self.iw_pos_x_value/(self.iw_big_size_value - self.overlap_size_value) + (self.iw_pos_y_value/(self.iw_big_size_value - self.overlap_size_value))*self.field_shape[1]].astype(
            np.float64), window_b=self.big_windows[self.iw_pos_x_value/(self.iw_big_size_value - self.overlap_size_value) + (self.iw_pos_y_value/(self.iw_big_size_value - self.overlap_size_value))*self.field_shape[1]].astype(np.float64), corr_method='fft', nfftx=None, nffty=None)

        self.corr_mat_plt = Window(
            self.calc_cor_matrix.shape[0], self.calc_cor_matrix.shape[1], self.calc_cor_matrix, self.main_slider_value)
    ...
    def moving_sub_window_array(self, array, window_size, overlap, subwindow_inset):
        ...
        sz = array.itemsize
        shape = array.shape
        subwindow_size = int(window_size - 2*subwindow_inset)
        assert(subwindow_size > 0)
        strides = (sz*(shape[1]*subwindow_inset+subwindow_inset), sz*shape[1]
                   * (window_size-overlap), sz*(window_size-overlap), sz*shape[1], sz)
        shape = (2, int((shape[0] - window_size)/(window_size-overlap))+1,
                 int((shape[1] - window_size)/(window_size-overlap))+1, subwindow_size, subwindow_size)
